...
class UserAdmin(admin.ModelAdmin):
    list_display = (
        'username', 'email', 'email_confirmed', 'confirmation_sent_at', 
        'profile_picture', 'failed_login_attempts', 'lockout_until'
    )
...
class CommentAdmin(admin.ModelAdmin):
    list_display = (
        'post', 'author', 'content', 'created_at', 'parent_comment'
    )
...
class SuggestionAdmin(admin.ModelAdmin):
    list_display = (
        'post', 'user', 'content', 'status', 'created_at', 'modified_at'
    )
    search_fields = ('content',)
...
class ReportAdmin(admin.ModelAdmin):
    list_display = (
        'report_type', 'post', 'comment', 'reported_by', 'reason', 
